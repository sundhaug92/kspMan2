...
namespace KerbalPackageManager
{
    internal class CkanRepository : Repository
    {
        ...
        public CkanRepository()
        {
            ...
            if (File.GetLastWriteTime(Path.Combine(".kpm", "cache", "ckan", "ckan-meta.zip")) < DateTime.Now.AddHours(-1))
            {
                try
                {
                    if (!Directory.Exists(Path.Combine(".kpm", "cache", "ckan"))) Directory.CreateDirectory(Path.Combine(".kpm", "cache", "ckan"));
                    wc.DownloadFile("https://github.com/KSP-CKAN/CKAN-meta/archive/master.zip", Path.Combine(".kpm", "cache", "ckan", "ckan-meta.zip"));
                }
                catch (WebException we)
                {
                    Console.WriteLine(we.Status);
                }
                using (Stream stream = File.OpenRead(Path.Combine(".kpm", "cache", "ckan", "ckan-meta.zip")))
                {
                    ZipArchive zip = new ZipArchive(stream);
                    string targetDirectory = Path.Combine(".kpm", "cache", "ckan", "ckan-meta");
